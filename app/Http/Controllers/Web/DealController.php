...
class DealController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Filtres (statut par défaut : deals ouverts)
        $statusFilter = in_array($request->get('status'), ['open', 'won', 'lost']) ? $request->get('status') : 'open';
        $stageId = $request->get('pipeline_stage_id');
        ...
        $cacheKey = 'deals.index.p'.$page.'.s'.$sort.'.d'.$dir.'.'.md5((string) $search)
            .'.st'.$statusFilter.'.ps'.$stageId.'.o'.$ownerId
            .'.sc'.$scopeKey;

        $cached = Cache::tags(['deals.index'])->remember($cacheKey, 30, function () use ($search, $sort, $dir, $page, $user, $statusFilter, $stageId, $ownerId) {
            $base = Deal::with([
                'stage:id,name',
                'companies:id,name',
                ...
                ->when($stageId, fn ($q) => $q->where('pipeline_stage_id', $stageId))
        ...
        return view('pages.deals.index', compact('deals', 'filter', 'sort', 'dir', 'search', 'allCount', 'total', 'stages', 'statusFilter', 'stageId', 'ownerId', 'owners'));
    }
    ...
    public function store(Request $request)
    {
        ...
        AssociationAuditor::recordAttach($deal, 'contacts', $contact->id, Contact::class, $contactPivot);
        ...
        if ($company) {
            ...
            AssociationAuditor::recordAttach($deal, 'companies', $company->id, Company::class, $companyPivot);
        }
    ...
    public function show(Request $request, Deal $deal)
    {
        ...
        $allContacts = Cache::tags(['contacts.index'])->remember('contacts.dropdown', 60, fn () => Contact::select('id', 'first_name', 'last_name')->orderBy('last_name')->get()
        );
    ...
    public function bulkDestroy(Request $request)
    {
        ...
        $count = Deal::whereIn('id', $data['ids'])
            ->visibleTo($user)
            ->delete();
    ...
    public function attachContact(Request $request, Deal $deal)
    {
        ...
        AssociationAuditor::recordAttach($deal, 'contacts', $contactId, Contact::class, $pivot);
    ...
    public function detachContact(Request $request, Deal $deal, Contact $contact)
    {
        // Cloisonnement par owner : 404 si le deal est hors du périmètre du commercial.
        $this->ensureVisible($deal, $request->user());
        ...
        AssociationAuditor::recordDetach($deal, 'contacts', $contact->id, Contact::class);
    ...
    public function attachCompany(Request $request, Deal $deal)
    {
        ...
        AssociationAuditor::recordAttach($deal, 'companies', $companyId, Company::class, $pivot);

        return redirect('/deals/'.$deal->id)
            ->with('flash_toast', ['message' => 'Entreprise associée au deal.', 'type' => 'success']);
    }

    public function detachCompany(Request $request, Deal $deal, Company $company)
    {
        // Cloisonnement par owner : 404 si le deal est hors du périmètre du commercial.
        $this->ensureVisible($deal, $request->user());
        ...
        AssociationAuditor::recordDetach($deal, 'companies', $company->id, Company::class);
